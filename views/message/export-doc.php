...
$table->addRow($n2sm);
insertText($table->addCell(1150, $styleHeaderCell), '№' . "\r\n" . 'Дата', $fontHeaderStyle);
insertText($table->addCell(3700, $styleHeaderCell), 'Фамилия Имя Отчество' . "\r\n" . 'Контакты', $fontHeaderStyle);
insertText($table->addCell(3700, $styleHeaderCell), 'Тема' . "\r\n" . 'Учреждение', $fontHeaderStyle);
insertText($table->addCell(7300, $styleHeaderCell), 'Обращение', $fontHeaderStyle);
...
//    Yii::info(str_repeat('-', 30) . ' Page: ' . $page . ' ['.$cou.']');
    ...
    foreach($dataProvider->getModels() As $model) {
        $table->addRow();
        insertText($table->addCell(1150, $styleCell), $model->msg_id . "\r\n" . date("d.m.Y", strtotime($model->msg_createtime)) . "\r\n" . $model->flag->fl_sname, $fontStyle);
        insertText($table->addCell(3700, $styleCell), $model->getFullName() . "\r\n" . $model->msg_pers_email . "\r\n" . $model->msg_pers_phone . "\r\n\r\n" . (($model->msg_empl_id !== null) ? $model->employee->getFullName() : ''), $fontStyle);
        insertText($table->addCell(3700, $styleCell), ($model->subject ? ($model->subject->tag_title . "\r\n") : '') . $model->msg_pers_org, $fontStyle);
        insertText($table->addCell(7300, $styleCell), $model->msg_pers_text, $fontStyle);
/*
